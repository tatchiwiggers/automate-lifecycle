...
def load_model(stage="Production") -> keras.Model:
    ...
    if MODEL_TARGET == "local":
        ...
        model = None
        # set tracking URI as always
        mlflow.set_tracking_uri(MLFLOW_TRACKING_URI)
        # create a client to interact with the server and pull th epath to out latest model
        client = mlflow.tracking.MlflowClient()
        #gets the latest version of ouor model
        version = client.get_latest_versions(MLFLOW_MODEL_NAME)
        #version is a list containing the ModelVersion Object.
        model_uri = version[0].source
        #here we load our mmodel
